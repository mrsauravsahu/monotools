...
calculate-version)
    ...
    if [ "${GITVERSION_REPO_TYPE}" = 'SINGLE_APP' ]; then
        ...
        ~/.dotnet/tools/dotnet-gitversion $(pwd) /config "${CONFIG_FILE}"
        gitversion_calc=$(~/.dotnet/tools/dotnet-gitversion $(pwd) /config "${CONFIG_FILE}")
        ...
        for svc in "${changed_services[@]}"; do
            ...
            ~/.dotnet/tools/dotnet-gitversion $(pwd) /config "/repo/${svc}/.gitversion.yml"
            gitversion_calc=$(~/.dotnet/tools/dotnet-gitversion $(pwd) /config "/repo/${svc}/.gitversion.yml")
            GITVERSION_TAG_PROPERTY_NAME="GITVERSION_TAG_PROPERTY_PULL_REQUESTS"
            GITVERSION_TAG_PROPERTY=${!GITVERSION_TAG_PROPERTY_NAME}
            service_version=$(echo "${gitversion_calc}" | jq -r "[${GITVERSION_TAG_PROPERTY}] | join(\"\")")
