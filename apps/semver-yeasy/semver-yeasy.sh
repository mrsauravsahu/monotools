...
update-pr)
    ...
    # awk script kept for reference
    # UPDATED_PR_BODY=$(gaqk -v semveryeasy_pr="$SEMVERYEASY_PR_BODY" -v start="$REGION_START" -v end="$REGION_END" '
    #     BEGIN {ins=1}
    #     {
    #         line=tolower($0);
    #         print "ins" ins
    ...
    if [[ -z "${RUN_ENV}" ]]; then
        ...
        ${JQ_EXEC_PATH} -nc --arg body "${UPDATED_PR_BODY_NEWLINES_REPLACED}" '{"body": $body}' | \
            curl -sL -X PATCH -d @- \
